...
export default function Login() {
  ...
  const handleSubmit = async (e: React.FormEvent<HTMLFormElement>) => {
    e.preventDefault();
    setIsLoading(true);
    // Simulate login process for a real estate rental platform
        ...
        <div className="hidden lg:block text-emerald-50 space-y-10 px-12">
          <div className="space-y-8">
            ...
            <div className="flex items-center gap-4">
              ...
              <p className="text-emerald-200 font-medium">Your Key to Premier Rentals</p>
            </div>
            {/* Headline */}
            <div className="space-y-6">
              ...
              <p className="text-xl text-emerald-100 leading-relaxed font-medium">
                Access real-time rental analytics, manage your listings with ease, and connect with qualified tenants on our industry-leading platform.
              </p>
            </div>
            {/* Features */}
            <div className="space-y-4">
              {[{
                icon: <Lock className="w-6 h-6 text-emerald-300" />, title: "Secure Access", desc: "Enterprise-grade security for your data", gradient: "from-green-600 to-emerald-600"
              }, {
                icon: <Mail className="w-6 h-6 text-emerald-300" />, title: "Instant Notifications", desc: "Stay updated with real-time alerts", gradient: "from-lime-500 to-green-500"
              }, {
                icon: <UserPlus className="w-6 h-6 text-emerald-300" />, title: "Easy Onboarding", desc: "Quick registration for new landlords & tenants", gradient: "from-teal-500 to-cyan-500"
              }].map((feature, idx) => (
            ...
            <div className="flex items-center gap-6 pt-4">
              <div className="text-center">
                ...
                <div className="text-sm text-emerald-200 font-medium">Available Listings</div>
              </div>
              <div className="w-px h-12 bg-emerald-700"></div>
              <div className="text-center">
                <div className="text-2xl font-black text-white">500K+</div>
                <div className="text-sm text-emerald-200 font-medium">Leases Signed</div>
              </div>
              <div className="w-px h-12 bg-emerald-700"></div>
              <div className="text-center">
                <div className="text-2xl font-black text-white">99.9%</div>
                <div className="text-sm text-emerald-200 font-medium">Uptime</div>
              </div>
            </div>
          </div>
        </div>
        ...
        <div className="w-full max-w-md mx-auto">
          <div className="bg-emerald-950/90 dark:bg-emerald-950/90 backdrop-blur-xl rounded-3xl shadow-2xl border border-emerald-800 overflow-hidden">
            ...
            <div className="bg-gradient-to-br from-emerald-900 via-green-900 to-emerald-800 p-8 text-white text-center relative overflow-hidden">
              <div className="absolute inset-0 bg-[url('data:image/svg+xml,%3Csvg width=\'40\' height=\'40\' viewBox=\'0 0 40 40\' xmlns=\'http://www.w3.org/2000/svg\'%3E%3Cg fill=\'%23ffffff\' fill-opacity=\'0.05\'%3E%3Cpath d=\'M20 20c0-5.5-4.5-10-10-10s-10 4.5-10 10 4.5 10 10 10 10-4.5 10-10zm10-10c0-5.5-4.5-10-10-10s-10 4.5-10 10 4.5 10 10 10 10-4.5 10-10z\'/%3E%3C/g%3E%3C/svg%3E')] opacity-30" />
              <div className="relative z-10">
                ...
                <h2 className="text-3xl font-black mb-3">Sign In to Bey</h2>
                <p className="text-white/80 font-medium">Access your rental management dashboard</p>
              </div>
            </div>
            {/* Form Body */}
            <form onSubmit={handleSubmit} className="p-8 space-y-6">
              ...
              <div className="flex items-center justify-between">
                <label className="flex items-center gap-3 cursor-pointer group">
                  <input
                    ...
                    className="sr-only"
                  />
                  <div className={`w-5 h-5 border-2 rounded flex items-center justify-center transition-all duration-200 ${rememberMe ? 'bg-emerald-700 border-emerald-700' : 'bg-emerald-900 border-emerald-800 group-hover:border-emerald-600'}`}>
                    <svg className={`w-3 h-3 text-white transition-opacity duration-200 ${rememberMe ? 'opacity-100' : 'opacity-0'}`} fill="currentColor" viewBox="0 0 20 20">
                      <path fillRule="evenodd" d="M16.707 5.293a1 1 0 010 1.414l-8 8a1 1 0 01-1.414 0l-4-4a1 1 0 011.414-1.414L8 12.586l7.293-7.293a1 1 0 011.414 0z" clipRule="evenodd"/>
                    </svg>
                  </div>
                  <span className="text-sm text-emerald-200 font-medium group-hover:text-white">Remember me</span>
                </label>
                <a href="/forgot-password" className="text-sm text-emerald-400 hover:text-emerald-200 font-semibold transition-colors duration-200 hover:underline">
                  Forgot password?
                </a>
              </div>
              {/* Submit Button */}
              ...
              >
                ...
                {isLoading ? (
                  ...
                  <span className="flex items-center justify-center gap-2"><LogIn className="w-5 h-5" />Sign In & Manage Rentals</span>
                )}
              </button>
              {/* Divider */}
              <div className="relative py-4">
                ...
                <div className="relative flex justify-center">
                  <span className="px-4 bg-emerald-950 text-emerald-300 font-medium">Or continue with</span>
                </div>
              </div>
              {/* Social Logins */}
              ...
              <div className="text-center pt-6 border-t border-emerald-800">
                <p className="text-emerald-200 font-medium">
                  New to Bey?{' '}
                  <Link href="/auth/register" className="text-emerald-400 hover:text-emerald-200 font-bold transition-colors duration-200 hover:underline flex items-center gap-1">
                    <UserPlus className="w-4 h-4 inline" />Create an account
                  </Link>
                </p>
              </div>
            </form>
          ...
          <div className="lg:hidden text-center mt-8">
            <div className="flex items-center justify-center gap-4">
              ...
              <span className="text-emerald-200 font-medium">Bey Rentals</span>
            </div>
          </div>
        </div>
      </div>
